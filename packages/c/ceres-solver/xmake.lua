package("ceres-solver")
    ...
    on_install("windows|x64", "windows|x86", "linux", "macosx", function (package)
        ...
        if package:is_plat("windows") then
            table.insert(configs, "-DMSVC_USE_STATIC_CRT=" .. ((package:config("runtimes") and package:has_runtime("MT", "MTd")) or (package:config("vs_config") and package:config("vs_config"):startswith("MT")) and "ON" or "OFF"))
        end
