...
target("hpsocket")
    ...
    if is_plat("windows") then
        ...
        ssllinkdir = path.join("Windows/Common/Lib/openssl", vs_ver, "$(arch)", "lib")
        add_linkdirs(ssllinkdir)
        add_includedirs(path.join("Windows/Common/Lib/zlib", vs_ver, "$(arch)", "include"))
        zliblinkdir = path.join("Windows/Common/Lib/zlib", vs_ver, "$(arch)", "lib")
