package("ultralight")
    ...
    if is_plat("windows") then
        if is_arch("x86_64", "x64") then
            ...
            add_versions("release:2023.07.24", "4fa7aadd1e4ba4a7dc04d17b1d82b37b141c6e4e7196501150486fa6ac1635c5", {version = function (version) return versions[tostring(version)] end})
        end
    elseif is_plat("linux") then
        if is_arch("x86_64", "x64") then
            add_urls("https://ultralight-sdk.sfo2.cdn.digitaloceanspaces.com/ultralight-sdk-$(version)-linux-x64.7z", {alias = "release"})
            add_versions("release:2023.07.24", "1de6298b5ed3c5e0c22ac27e0e30fcb0ba6d195467a58ee44ef4e13dd1a6d352", {version = function (version) return versions[tostring(version)] end})
        end
    elseif is_plat("macosx") then
        if is_arch("x86_64", "x64") then
            add_urls("https://ultralight-sdk.sfo2.cdn.digitaloceanspaces.com/ultralight-sdk-$(version)-mac-x64.7z", {alias = "release"})
            add_versions("release:2023.07.24", "bbf81ed456a617a60a19e9a76946e4479d5bac877f859005c50f66e9ec3c77a2", {version = function (version) return versions[tostring(version)] end})
        end
    end
