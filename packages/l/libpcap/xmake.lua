package("libpcap")
    ...
    --- define _GNU_SOURCE to enable GNU extension `asprintf`
    ...
    if on_check then
        on_check("mingw", function (package)
            assert(package:config("shared"), "package(libpcap): shared library is required for mingw")
        end)
    end
    ...
    on_install("linux", "macosx", "android", "bsd", "windows", "mingw", function (package)
        local configs = {
            "-DDISABLE_AIRPCAP=ON",
            "-DDISABLE_DPDK=ON",
            "-DDISABLE_NETMAP=ON",
