            ...
            if package:config(opt.conf) then
                ...
                if lib and not lib:is_system() then
                    ...
                    if fetchinfo then
                        ...
                        local libfiles = table.remove_if(table.clone(fetchinfo.libfiles or {}), function (file) return print(" remove pred", file, path.extension(file)) path.extension(file) == ".dll" end)
                        print(libfiles)
                        if #libfiles > 0 then
