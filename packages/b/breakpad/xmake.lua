package("breakpad")
    ...
    on_install("windows|x64", "windows|x86", "linux", function (package)
        ...
        if package:is_plat("linux") then
            ...
            table.insert(files, "src/client/minidump_file_writer.cc")
            table.insert(files, "src/client/linux/handler/exception_handler.cc")
            table.insert(files, "src/client/linux/minidump_writer/linux_dumper.cc")
            table.insert(files, "src/client/linux/minidump_writer/minidump_writer.cc")
            table.insert(files, "src/client/linux/minidump_writer/linux_ptrace_dumper.cc")
            table.insert(files, "src/client/linux/crash_generation/crash_generation_client.cc")
            table.insert(files, "src/client/linux/minidump_writer/cpu_set.h")
            table.insert(files, "src/client/linux/minidump_writer/line_reader.h")
            table.insert(files, "src/client/linux/minidump_writer/directory_reader.h")
            table.insert(files, "src/client/linux/minidump_writer/proc_cpuinfo_reader.h")
        end
