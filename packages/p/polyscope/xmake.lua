package("polyscope")
    ...
    add_patches("1.3.0", path.join(os.scriptdir(), "patches", "v1.3.0", "deps.patch"), "be68f899c68450d761d2db5658df57d718953e4b98b9fa6eee0e30bd2cd6aa5f")
    ...
    add_deps("glad", "glfw", "glm", "happly", "nlohmann_json")
    on_install("windows", "macosx", "linux", function (package)
        io.replace("deps/CMakeLists.txt", "add_subdirectory(glfw)", "find_package(GLFW REQUIRED)", {plain = true})
        io.replace("deps/CMakeLists.txt", "add_subdirectory(imgui)", "find_package(imgui REQUIRED)", {plain = true})
        local configs = {"-DPOLYSCOPE_BACKEND_OPENGL_MOCK=OFF"}
        table.insert(configs, "-DCMAKE_BUILD_TYPE=" .. (package:debug() and "Debug" or "Release"))
        table.insert(configs, "-DBUILD_SHARED_LIBS=" .. (package:config("shared") and "ON" or "OFF"))
        import("package.tools.cmake").install(package, configs, {packagedeps = {"imgui", "glad", "glfw", "glm", "happly", "nlohmann_json"}})
