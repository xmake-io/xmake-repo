...
function _generate_package_from_gitlab(reponame)
    -- get repository info
    local glab = assert(find_tool("glab"), "glab not found!")
    local glab_host = os.iorunv(glab.program, { "config", "get", "host" }):trim()
    print("glab host: %s", glab_host)
    local repoinfo = os.iorunv(glab.program, {
        "api",
        "graphql",
        "-f",
        "query={ project(fullPath: \""..reponame.."\") { description webUrl sshUrlToRepo name } }",
    })
    if repoinfo then
        repoinfo = json.decode(repoinfo)
        if repoinfo and repoinfo.data then
            repoinfo = repoinfo.data.project
            -- restructuring the data
            local new_repoinfo = {}
            new_repoinfo.description = repoinfo.description
            new_repoinfo.homepageUrl = repoinfo.webUrl
            new_repoinfo.licenseInfo = "MIT"
            new_repoinfo.url = repoinfo.webUrl
            new_repoinfo.sshUrl = repoinfo.sshUrlToRepo
            new_repoinfo.name = repoinfo.name
            repoinfo = new_repoinfo
        end
    end
    print(repoinfo)
    ...
    if type(latest_release) == "table" then
        local url = ("https://%s/%s/-/archive/%s.tar.gz"):format(glab_host, reponame, latest_release.tagName)
        local giturl = ("https://%s/%s.git"):format(glab_host, reponame)
        -- file:write('    add_urls("https://gitlab.com/' .. reponame .. '/-/archive/$(version).tar.gz",\n')
        file:write(('    add_urls("https://%s/%s/-/archive/$(version).tar.gz",\n'):format(glab_host, reponame))
        file:print('             "%s")', giturl)
        ...
        -- local giturl = ("https://%s/%s.git"):format(glab_host, reponame)
        local giturl = ("git@%s:%s.git"):format(glab_host, reponame)
...
function _generate_package_from_github(reponame)
    ...
    local repoinfo = os.iorunv(gh.program, {
        "repo",
        "view",
        reponame,
        "--json",
...
function main(...)
    ...
    elseif repo and repo:startswith("gitlab:") then
        _generate_package_from_gitlab(repo:sub(8))
    else
